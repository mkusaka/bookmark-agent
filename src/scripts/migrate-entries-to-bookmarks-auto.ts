...
async function phase2CopyData() {
  ...
  try {
    ...
    `);
    const missingData = missingResult.rows[0] as { count: number };
...
async function verifyMigration() {
  ...
  `);
  const completeBookmarks = completeResult.rows[0] as { count: number };

  const totalResult = await db.execute(sql`
    SELECT COUNT(*) as count
    FROM bookmarks
  `);
  const totalBookmarks = totalResult.rows[0] as { count: number };
...
async function main() {
  console.log('🚀 Bookmark-Entry Table Migration Tool (Automatic)\n');
